...
void CGameClient::doRender()
{
    ...
    sf::Shader *pShaderCAbb = Assets().getShader(CAssetManager::SHADER_CHROMATIC_ABERRATION);
    pShaderCAbb->setUniform("texture", sf::Shader::CurrentTexture);
    pShaderCAbb->setUniform("offc", sf::Vector3f(0.01f, 0.01f, 0.01f));
	...
	if (pShaderBloom)
	{
		...
		{
			pShaderBloom->setUniform("direction", (i%2 == 0)?sf::Vector2f(sIters-i-1, 0):sf::Vector2f(0, sIters-i-1));
...
//		if (pShaderCAbb)
//		{
//			m_RenderPhaseTextureFinal.draw(m_RenderPhase, pShaderCAbb);
//			m_RenderPhaseTextureFinal.display();
//			m_RenderPhase.setTexture(&m_RenderPhaseTexture.getTexture());
//
//			m_RenderPhaseTexture.draw(m_RenderPhase);
...
bool CGameClient::init() noexcept
{
	...
	m_vpComponents.push_back(&m_EnemyRender);
	...
	m_vpSystems.push_back(&m_SystemFMod); 		// FMod: sound analysis

	m_SystemSound.setListenerPosition(sf::Vector2f(0.0f, 0.0f));
