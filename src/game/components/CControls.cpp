...
{
	setKeyBind("lineRed", sf::Keyboard::A);
	setKeyBind("lineGreen", sf::Keyboard::S);
	setKeyBind("lineBlue", sf::Keyboard::D);
}
	...
	if (ev.type == sf::Event::KeyPressed)
	{
		if (ev.key.code == sf::Keyboard::Escape)
		{
			...
			if (Client()->Menus().getActiveModal() == CMenus::NONE || Client()->Menus().getActiveModal() == CMenus::MODAL_GAMEOVER)
			{
				...
				{
					Client()->initializeGameMode("menu");
